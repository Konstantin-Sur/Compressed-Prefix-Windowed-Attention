"""
Train a CPWA (Compressed-Prefix Sliding Window Attention) language model on
the Isotonic/human_assistant_conversation dataset.
...
Data change vs. the FineWeb version:
- Streams "Isotonic/human_assistant_conversation" (train split) instead of FineWeb.
- Uses the dataset's "text" field, which already looks like:
    ####Human####:  ...  ####Assistant####:  ... <|stop|>
- Every literal "<|stop|>" in that text is replaced with "<|endoftext|>"
  before tokenization, so the GPT-2 tokenizer's eot_token is used as the
  turn/example terminator instead of a literal "<|stop|>" string (which
  isn't a special token in the gpt2 tiktoken encoding anyway).
"""
...
CONFIG = {
    ...
    "dataset_subset": None,          # this dataset has no subset/config name
    ...
    "checkpoint_path": "/content/model_cpwa_human_assistant.pt",
}
...
# ========== DATA LOADER ==========
class HumanAssistantStream(IterableDataset):
    ...
    def __iter__(self):
        ...
        for example in ds:
            ...
            # Swap the literal stop marker for the real EOT marker.
...
def make_loader(split):
    dataset = HumanAssistantStream(seq_len=CONFIG["seq_len"], split=split)
    return DataLoader(dataset, batch_size=CONFIG["batch_size"])

# The dataset has "train" and "test" splits; use "train" for both here (same
# pattern as the original script, which used the same subset for train/val).
train_loader = make_loader("train")
val_loader = make_loader("train")
...
def train(resume_from=None):
    ...
    start_step = 0 # BECAUSE THIS IS "fine tune" AND DO NOT REQUIRE EXACT CURSOR POSITION
...
####Assistant####:""", max_new_tokens=1024):
    ...
    out = model.generate(idx, max_new_tokens=max_new_tokens, temperature=0.7)
...
if __name__ == "__main__":
    train(resume_from="/content/drive/MyDrive/CPWA Assistant 75M.pt")  # start fresh; pass resume_from="/path/to/checkpoint.pt" to resume
    print("\n--- sample generation ---")
    print(sample())
